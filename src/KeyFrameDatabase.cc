...
namespace ORB_SLAM3
{
    ...
    // default accuracyFactor is 0.75, lowering this should make candidates with lower scores acceptable, therefore increase candidate number while reducing accuracy
    vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F, Map *pMap, const float accuracyFactor = 0.75f, const float minCommonWordsFactor = 0.8f)
    {
        list<KeyFrame *> lKFsSharingWords;
        ...
        int minCommonWords = maxCommonWords * 0.8f;
        if (minCommonWordsFactor >= 0 && minCommonWordsFactor <= 1)
        {
            minCommonWords = maxCommonWords * minCommonWordsFactor;
        }
        ...
        // default: Return all those keyframes with a score higher than 0.75*bestScore
        float minScoreToRetain = 0.75f * bestAccScore;
        if (accuracyFactor <= 1 && accuracyFactor >= 0)
        {
            minScoreToRetain = accuracyFactor * bestAccScore;
        }
