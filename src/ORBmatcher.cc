...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchByBoW(const AuxiliaryFrame &refAuxFrame, const Frame &refCurrentFrame, vector<MapPoint *> &vpMapPointMatches)
    {
        const auto &vpMapPointsKF = refAuxFrame.GetFrame().GetMapPoints();

        vpMapPointMatches = vector<MapPoint *>(refCurrentFrame.N, static_cast<MapPoint *>(NULL));

        // calculate auxiliary BoW and FeatVec
        const AuxiliaryFrame::BoWAndFeatureVecs currentFrameAuxBowAndFeatVecs = AuxiliaryFrameDatabase::getAuxiliaryBoW(refCurrentFrame, AuxiliaryFrameDatabase::getVocTreeLevelsUp());

        int nmatches = 0;

        const auto &vFeatVecKF = refAuxFrame.mBoWAndFeatureVecs.mAuxFeatVec;
        const auto &currentFrameFeatVec = currentFrameAuxBowAndFeatVecs.mAuxFeatVec;
        ...
        // We perform the matching over ORB that belong to the same vocabulary node (at a certain level)
        DBoW2::FeatureVector::const_iterator KFit = vFeatVecKF.begin();
        DBoW2::FeatureVector::const_iterator Fit = currentFrameFeatVec.begin();
        DBoW2::FeatureVector::const_iterator KFend = vFeatVecKF.end();
        DBoW2::FeatureVector::const_iterator Fend = currentFrameFeatVec.end();
        ...
        while (KFit != KFend && Fit != Fend)
        {
            if (KFit->first == Fit->first)
            {
                ...
                for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++)
                {
                    ...
                    const cv::Mat &dKF = refAuxFrame.GetFrame().mDescriptors.row(realIdxKF);
                    ...
                    for (size_t iF = 0; iF < vIndicesF.size(); iF++)
                    {
                        if (refCurrentFrame.Nleft == -1)
                        {
                            ...
                            const cv::Mat &dF = refCurrentFrame.mDescriptors.row(realIdxF);
                        ...
                        else
                        {
                            ...
                            if (realIdxF < refCurrentFrame.Nleft && dist < bestDist1)
                            {
                                bestDist2 = bestDist1;
                                bestDist1 = dist;
                                bestIdxF = realIdxF;
                            }
                            else if (realIdxF < refCurrentFrame.Nleft && dist < bestDist2)
                            {
                                bestDist2 = dist;
                            }

                            if (realIdxF >= refCurrentFrame.Nleft && dist < bestDist1R)
                            {
                                bestDist2R = bestDist1R;
                                bestDist1R = dist;
                                bestIdxFR = realIdxF;
                            }
                            else if (realIdxF >= refCurrentFrame.Nleft && dist < bestDist2R)
                            {
                                bestDist2R = dist;
                            }
                        }
                    ...
                    if (bestDist1 <= TH_LOW)
                    {
                        if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2))
                        {
                            ...
                            const cv::KeyPoint &kp = refAuxFrame.GetFrame().mvKeysUn[realIdxKF];

                            if (mbCheckOrientation)
                            {
                                const cv::KeyPoint &Fkp = refCurrentFrame.mvKeys[bestIdxF];
                        ...
                        if (bestDist1R <= TH_LOW)
                        {
                            if (static_cast<float>(bestDist1R) < mfNNratio * static_cast<float>(bestDist2R) || true)
                            {
                                ...
                                if (mbCheckOrientation)
                                {
                                    const cv::KeyPoint &Fkp = refCurrentFrame.mvKeys[bestIdxFR];
            ...
            {
                Fit = currentFrameFeatVec.lower_bound(KFit->first);
            }
        }
    ...
    int ORBmatcher::calculateHistBinIndexFromRotation(float rot)
    {
        if (rot < 0.0)
            rot += 360.0f;
        int bin = round(rot / (360.0f / HISTO_LENGTH));
        int binIndex;
        if (bin != 0)
        {
            binIndex = bin - 1;
        ...
        assert(binIndex >= 0 && binIndex < HISTO_LENGTH);
        return binIndex;
    }
