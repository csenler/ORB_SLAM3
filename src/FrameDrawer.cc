...
namespace ORB_SLAM3
{
    ...
    cv::Mat FrameDrawer::DrawFrame(float imageScale)
    {
        ...
        std::vector<cv::KeyPoint> vMarkerKeyPoints;
        vector<bool> vbVO, vbMap, vbMarkers; // Tracked MapPoints in current frame
        ...
        cv::Scalar markerColor(128, 0, 128); // purple
        ...
        // Copy variables within scoped mutex
        {
            ...
            if (mState == Tracking::NOT_INITIALIZED)
            {
                ...
                for (const auto &kv : mmMarkerKeys)
                {
                    if (kv.first.first >= 0)
                    {
                        // value will be keypoint
                        vMarkerKeyPoints.push_back(kv.second);
                    }
                }
            }
        ...
        else if (state == Tracking::OK) // TRACKING
        {
            ...
            int nCurrentKeys = vCurrentKeys.size();
            int n = vCurrentKeys.size() + vMarkerKeyPoints.size();
            std::cout << "FrameDrawer DrawFrame -> vCurrentKeys.size() " << vCurrentKeys.size() << " vMarkerKeyPoints.size() " << vMarkerKeyPoints.size() << " n: " << n << "\n";
            std::cout << "mvbMarkers.size() " << mvbMarkers.size() << " vbMarkers.size() " << vbMarkers.size() << std::endl;
            if (mvbMarkers.size() != n)
            {
                std::cout << "mvbMarkers.size() " << mvbMarkers.size() << " n " << n << std::endl;
            }
            for (int i = 0; i < n; i++)
            {
                if (i < nCurrentKeys)
                ...
                else
                {
                    if (vbMarkers[i])
                    {
                        ...
                        if (imageScale != 1.f)
                        {
                            point = vMarkerKeyPoints[i - nCurrentKeys].pt / imageScale;
                            float px = vMarkerKeyPoints[i - nCurrentKeys].pt.x / imageScale;
                            float py = vMarkerKeyPoints[i - nCurrentKeys].pt.y / imageScale;
                            pt1.x = px - r;
                            pt1.y = py - r;
                            pt2.x = px + r;
                            pt2.y = py + r;
                        ...
                        {
                            point = vMarkerKeyPoints[i - nCurrentKeys].pt;
                            pt1.x = vMarkerKeyPoints[i - nCurrentKeys].pt.x - r;
                            pt1.y = vMarkerKeyPoints[i - nCurrentKeys].pt.y - r;
                            pt2.x = vMarkerKeyPoints[i - nCurrentKeys].pt.x + r;
                            pt2.y = vMarkerKeyPoints[i - nCurrentKeys].pt.y + r;
                        }
                        cv::rectangle(im, pt1, pt2, markerColor);
                        cv::circle(im, point, 2, markerColor, -1);
                    }
                }
            }
        }
    ...
    void FrameDrawer::Update(Tracking *pTracker)
    {
        ...
        mmMarkerKeys.clear();
        for (const auto &marker : pTracker->mCurrentFrame.mvMarkers)
        {
            if (marker.id >= 0)
            {
                int cornerIndex = 0;
                for (const auto &corner : marker.cornerKeyPoints)
                {
                    mmMarkerKeys[make_pair(marker.id, cornerIndex)] = corner;
                    cornerIndex++;
                }
            }
        }

        if (mmMarkerKeys.size() == 1)
        {
            std::cout << "FrameDrawer Update -> mmMarkerKeys.size() " << mmMarkerKeys.size() << std::endl;
        }

        const auto iCurrentKeysSize = mvCurrentKeys.size();
        ...
        else
        {
            ...
            N = mvCurrentKeys.size() + mmMarkerKeys.size();
        }
        // std::cout << "FrameDrawer Update -> mvCurrentKeys.size() " << mvCurrentKeys.size() << " mmMarkerKeys.size() " << mmMarkerKeys.size() << " N: " << N << std::endl;

        mvbVO = vector<bool>(N, false);
        mvbMap = vector<bool>(N, false);
        mbOnlyTracking = pTracker->mbOnlyTracking;

        mvbMarkers = vector<bool>(N, false);
        ...
        else if (pTracker->mLastProcessedState == Tracking::OK)
        {
            for (int i = 0; i < N; i++)
            {
                ...
                if (pMP)
                {
                    const int iMarkerId = pMP->mnMarkerId.load();
                    const int iMarkerCornerId = pMP->mnMarkerCornerId.load();
                    const bool bMarkerFlag = iMarkerId >= 0 && iMarkerCornerId >= 0;
                    ...
                    if (!pTracker->mCurrentFrame.mvbOutlier[i])
                    {
                        ...
                        if (bMarkerFlag)
                        {
                            mmMatchedInImage[pMP->mnId] = mmMarkerKeys[make_pair(iMarkerId, iMarkerCornerId)].pt;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (bMarkerFlag)
                        {
                            mvOutlierKeys.push_back(mmMarkerKeys[make_pair(iMarkerId, iMarkerCornerId)]);
                            std::cout << "outlier corner for -> marker " << iMarkerId << " corner " << iMarkerCornerId << std::endl;
                        }
                        else
                        {
