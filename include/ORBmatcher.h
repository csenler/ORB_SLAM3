...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        int SearchByProjection(Frame &CurrentFrame, const Frame &refFrame, const set<MapPoint *> &sAlreadyFound, const float th, const int ORBdist);
        // (for use with aux db) search without excluding any points found before
        int SearchByProjection(Frame &CurrentFrame, const Frame &refFrame, const float th, const int ORBdist);
        ...
        static void changeORBMatcherHighThreshold(const int thHigh_)
        {
            ORBmatcher::TH_HIGH = thHigh_;
        };

        static void changeORBMatcherLowThreshold(const int thLow_)
        {
            ORBmatcher::TH_LOW = thLow_;
        };

        static int getORBMatcherHighThreshold()
        {
            return ORBmatcher::TH_HIGH;
        };

        static int getORBMatcherLowThreshold()
        {
            return ORBmatcher::TH_LOW;
        }
